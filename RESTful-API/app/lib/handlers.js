...
// HTML Handlers
var htmlHandlers = require('./handlers/htmlHandlers');

// JSON API Handlers
var jsonApiHandlers = require('./handlers/jsonApiHandlers');

// JS Files
var ping = require('./handlers/ping');
var notFound = require('./handlers/notFound');
...
// Index handler
handlers.index = htmlHandlers.index;

// Create Account
handlers.accountCreate = htmlHandlers.accountCreate;

// Create New Session
handlers.sessionCreate = htmlHandlers.sessionCreate;

// Session has been deleted
handlers.sessionDeleted = htmlHandlers.sessionDeleted;

// Edit your account
handlers.accountEdit = htmlHandlers.accountEdit;

// Account has been deleted
handlers.accountDeleted = htmlHandlers.accountDeleted;

// Create a new check
handlers.checksCreate = htmlHandlers.checksCreate;

// Dashboard (view all checks)
handlers.checksList = htmlHandlers.checksList;

// Edit a check
handlers.checksEdit = htmlHandlers.checksEdit;

// Favicon
handlers.favicon = htmlHandlers.favicon;

// Public assets
handlers.public = htmlHandlers.public;
                                        ...
                                        _data.delete('checks', checkId, function (err) {
                                            ...
                                            if (checksDeleted == checksToDelete) {
                                                if (!deletionError) {
                                                    ...
                                                    callback(500, { 'Error': 'Errors encountered while attempting to delete all the user\'s checks. All checks may not have deleted from the system successfully' })
                                                }
                                            }
                                        });
                                    });
            ...
            if (!err && userData) {
                ...
                if (hashedPassword == userData.hashedPassword) {
                    ...
                    _data.create('tokens', tokenId, tokenObject, function (err) {
                        ...
                        _performance.measure('Token data creating', 'creating data for token', 'begining storing token');
        ...
        _data.read('tokens', id, function (err, tokenData) {
            if (!err && tokenData) {
                ...
                if (tokenData.expires > Date.now()) {
                    ...
                    callback(400, { 'Error': 'The token has alread expired, and connot be extended' });
                }
            } else {
                ...
                _data.read('users', userPhone, function (err, userData) {
                    if (!err && userData) {
                        ...
                        if (userChecks.length < config.maxChecks) {
                            ...
                            // Verify that url given has DNS entries (and threfore can resolve)
                        ...
                        callback(403, { 'Error': '403 One' });
                    }
                });
            } else {
                callback(403, { 'Error': '403 Two' });
            }
        });
    } else {
                                ...
                                _data.read('users', checkData.userPhone, function (err, userData) {
                                    if (!err && userData) {
                                        ...
                                        if (checkPosition > -1) {
                                            ...
                                            callback(500, { 'Error': 'Could not find the check on the user find, so could not remove it' });
                                        }
                                    } else {
                                        callback(400, { 'Error': 'Could not find the user who created the check, so could not remove the checks from the list of user object' });
                                    };
                                });
                            } else {
