            ...
            checks.forEach(function(check) {
                ...
                _data.read('checks', check, function(err, originalCheckData) {
                    if (!err && originalCheckData) {
                        // Pass it to the check validatior, and let that function continue or log errors as needed
...
workers.validateCheckData = function(originalCheckData) {
    ...
    // Set the kes that may not be set (if the workers have never seen this check before)
    ...
    // If all the checks pass, pass the data along th the next step in the process
    if (
        originalCheckData.id &&
        originalCheckData.userPhone &&
        originalCheckData.protocol &&
...
workers.performCheck = function(originalCheckData) {
    ...
    var req = _moduleToUse.request(requestDetails, function(res) {
        ...
        // Update the checoutcome and passs the data along
...
// Process the check outcome, update the check data as needed, trigger an alaert if needed
...
workers.alertUserToStatusChange = function(newCheckData) {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, function(err) {
        if (!err) {
            ...
            console.log('Error: Could not send sms alert to user who had a state change in their check. newCheckData: ', msg);
        }
    });
};
